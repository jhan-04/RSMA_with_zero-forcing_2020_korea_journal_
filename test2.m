%%%%%%%%%%%%%  Rate-Splitting Unifying SDMA, OMA, NOMA, and Multicasting in MISO Broadcast Channel_ A Simple Two-User Rate Analysis
%%%%%%%%%%%%% Fig 3 (a)
clear all
%clc
nt=2;%number of transmitter antenna
P=100;
i=0;
tic;
o=0;
%for gam_dB=-20:1:0
gam_dB=-5;
gam_dB
i=i+1;
j=0;
% k=0;
for  rho=0.001:0.1:1
    tic;
    o=o+1;
    j=j+1;
   
    %rho=0.03;
     rho
    [MA_p(i,j),tou_p(i,j), P1_p(i,j),P2_p(i,j), Pc_p(i,j),Rs_p(i,j)]=RS_paper(gam_dB,rho,P);
    y=[MA_p(i,j),tou_p(i,j), P1_p(i,j),P2_p(i,j), Pc_p(i,j),Rs_p(i,j)]
    [MA_x(i,j),tou_x(i,j), P1_x(i,j),P2_x(i,j), Pc_x(i,j),Rs_x(i,j),Rs_noma(i,j)]=RS_summaxrate(gam_dB,rho,P);
    x= [MA_x(i,j),tou_x(i,j), P1_x(i,j),P2_x(i,j), Pc_x(i,j),Rs_x(i,j),Rs_noma(i,j)]
    [MA_SDMA(i,j),tou_SDMA(i,j), P1_SDMA(i,j),P2_SDMA(i,j), Pc_SDMA(i,j),Rs_SDMA(i,j)]=RS_SDMA(gam_dB,rho,P);
   a=[MA_SDMA(i,j),tou_SDMA(i,j), P1_SDMA(i,j),P2_SDMA(i,j), Pc_SDMA(i,j),Rs_SDMA(i,j)]
    
  %  [MA_noma(i,j),tou_noma(i,j), P1_noma(i,j),P2_noma(i,j), Pc_noma(i,j),Rs_noma(i,j)]=RS_noma(gam_dB,rho,P);
  %  b=[MA_noma(i,j),tou_noma(i,j), P1_noma(i,j),P2_noma(i,j), Pc_noma(i,j),Rs_noma(i,j)]
    
    [MA_oma(i,j),tou_oma(i,j), P1_oma(i,j),P2_oma(i,j), Pc_oma(i,j),Rs_oma(i,j)]=RS_oma(gam_dB,rho,P);
    c=[MA_oma(i,j),tou_oma(i,j), P1_oma(i,j),P2_oma(i,j), Pc_oma(i,j),Rs_oma(i,j)]
    [MA_oma_ZF(i,j),tou_oma_ZF(i,j), P1_oma_ZF(i,j),P2_oma_ZF(i,j), Pc_oma_ZF(i,j),Rs_oma_ZF(i,j)]=RS_oma_ZF(gam_dB,rho,P);
  [MA_mul(i,j),tou_mul(i,j), P1_mul(i,j),P2_mul(i,j), Pc_mul(i,j),Rs_mul(i,j)]=RS_multi(gam_dB,rho,P);
     
     toc;
end



rho=0.01:0.1:1;
hold on
plot(rho,Rs_x(6,:),'r-o','MarkerIndices',1:round(length(rho)/10):length(rho))
plot(rho,Rs_SDMA,'c-.+','MarkerIndices',1:round(length(rho)/10):length(rho))
plot(rho,Rs_noma,'-.b^','MarkerIndices',1:round(length(rho)/10):length(rho))
plot(rho,Rs_oma,'-.g>','MarkerIndices',1:round(length(rho)/10):length(rho))

plot(rho,Rs_oma_Z,':m<','MarkerIndices',1:round(length(rho)/10):length(rho))
plot(rho,Rs_mul,':yh','MarkerIndices',1:round(length(rho)/10):length(rho))

plot(rho,Rs_p(6,:),':k*','MarkerIndices',1:round(length(rho)/10):length(rho))

legend('RS','SDMA','NOMA','OMA without ZF','OMA','pre-RS')
xlabel('rho')
ylabel('Sum of rate(bits/s/Hz)')
title(' SNR=20dB, -5dB(channel strength disparity)')

%end
hold off





% 
% hold on
% gam_dB=-20:1:0;
% %plot(rho,Rs_x,'r-o',rho,Rs_SDMA,'c:*',rho,Rs_noma,'-+b',rho,Rs_oma,'*:g')
% %plot(gam_dB,Rs_x,'r-',gam_dB,Rs_SDMA,'c-.',gam_dB,Rs_noma,'-.b',gam_dB,Rs_oma,'-.g')
% 
% plot(gam_dB,Rs_x,'r-o','MarkerIndices',1:round(length(gam_dB)/10):length(gam_dB))
% plot(gam_dB,Rs_SDMA,'c-.+','MarkerIndices',1:round(length(gam_dB)/10):length(gam_dB))
% plot(gam_dB,Rs_noma,'-.b^','MarkerIndices',1:round(length(gam_dB)/10):length(gam_dB))
% plot(gam_dB,Rs_oma,'-.g>','MarkerIndices',1:round(length(gam_dB)/10):length(gam_dB))
%plot(rho,Rs_oma_Z,':m<','MarkerIndices',1:round(length(gam_dB)/10):length(gam_dB))

% plot(gam_dB,Rs_p,':k*','MarkerIndices',1:round(length(gam_dB)/10):length(gam_dB))


% legend('RS','SDMA','NOMA','OMA','OMA_ZF','pre-RS')
% xlabel('channel strength disparity  [dB] (rho=0.03,P=10W)')
% ylabel('Sum of rate(bits/s/Hz)')
% %end
% 
% xlabel('rho')
%ylabel('channel strength disparity  [dB]')
% 
% hold off
% %end
% 
% 
% hold off